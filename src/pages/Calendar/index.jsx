...
const AgendaViewItem = ({ event, onEventClick, onUpdateEvent, employeeOptions }) => {
  const [{ isDragging }, drag] = useDrag({
    type: 'event',
    item: { event },
    collect: (monitor) => ({
        ...
        <div className="flex-1">
          ...
          {event.assignedEmployees.length > 0 && (
            <div className="flex items-center mt-1 text-sm text-muted-foreground">
              ...
              <span>
                {event.assignedEmployees.map(empId => {
...
const Events = () => {
  ...
  // Convert employees to the format expected by the form
  ...
  // System Integration: Check if an employee has conflicting events
  const checkEmployeeConflicts = (employeeId, startDate, endDate, startTime, endTime, currentEventId = null) => {
    ...
    return events.some(event => {
      ...
      // Check if the employee is assigned to this event
      if (!event.assignedEmployees.includes(employeeId)) return false;
      ...
      const sampleEvents = [
        {
          ...
          description: 'Daily morning shift for baristas',
          color: '#10B981'
        },
        {
          id: 2,
        ...
        },
        {
          ...
          endTime: '23:59',
          assignedEmployees: [],
          description: 'Company annual holiday',
          color: '#8B5CF6'
        }
      ];
  ...
  // Save events to localStorage and broadcast changes for real-time updates
  useEffect(() => {
    localStorage.setItem('calendarEvents', JSON.stringify(events));
    
    // Broadcast event changes to other tabs/windows
    window.dispatchEvent(new CustomEvent('eventsChanged', { detail: events }));
  }, [events]);
  ...
  // Auto-update current date to reflect today
  useEffect(() => {
    const timer = setInterval(() => {
      const now = new Date();
      // Only update if we're on day view and viewing today
      if (view === 'day' && isSameDay(currentDate, new Date())) {
        setCurrentDate(now);
      }
  ...
  const hasOverlappingEvent = (startDate, endDate, startTime, endTime, assignedEmployees, currentEventId = null) => {
    const startDateTime = new Date(`${startDate}T${startTime}`);
    const endDateTime = new Date(`${endDate}T${endTime}`);
    ...
    return events.some(event => {
      ...
      // Check if any of the assigned employees overlap
      const hasCommonEmployee = event.assignedEmployees.some(empId => 
        assignedEmployees.includes(empId)
      );
      
      if (!hasCommonEmployee) return false;
  ...
  const saveEvent = () => {
    ...
    if (hasOverlappingEvent(
      ...
      eventForm.startTime, 
      eventForm.endTime, 
      eventForm.assignedEmployees,
      selectedEvent?.id
    )) {
      alert('Selected employees have overlapping events during this time!');
      return;
    }
    ...
    if (selectedEvent) {
      ...
      newEvent = {
        ...selectedEvent,
        ...eventForm,
        assignedEmployees: eventForm.assignedEmployees
      };
  ...
  const deleteEvent = (eventId) => {
    ...
    if (window.confirm('Are you sure you want to delete this event?')) {
      ...
      // Real-time update notification
      console.log(`Event deleted: ${eventToDelete?.title || 'Unknown'}`);
    }
  };
  ...
  const updateEvent = (updatedEvent) => {
    ...
    if (hasOverlappingEvent(
      ...
      updatedEvent.assignedEmployees,
      updatedEvent.id
    )) {
      alert('Selected employees have overlapping events during this time!');
      return;
    ...
    // Real-time update notification
    console.log(`Event moved: ${updatedEvent.title}`);
  };
          ...
          <div className="pb-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <p className="text-muted-foreground mt-1">Manage your personal events</p>
              </div>
              <button 
                onClick={() => {
            ...
            <div className="bg-card rounded-xl border shadow-2xl max-w-md w-full flex flex-col max-h-[70vh]">
              ...
              <div className="overflow-y-auto flex-1 p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2 bg-gradient-to-br from-card/50 to-card border rounded-xl p-4 shadow-sm">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="p-2 rounded-lg bg-green-500/10">
                        <Users className="w-4 h-4 text-green-500" />
                      </div>
                      <h3 className="font-semibold text-base text-foreground">Assign Employees</h3>
                    </div>
                    <div className="space-y-1 max-h-32 overflow-y-auto border rounded-lg p-2 bg-muted/20">
                      {employeeOptions.map(employee => {
                        // Check if this employee has a conflict with the current event
                        const hasConflict = eventForm.startDate && 
                        ...
                        return (
                          <label key={employee.id} className={`flex items-center justify-between p-2 rounded-lg transition-colors ${hasConflict ? 'bg-destructive/10 text-destructive' : 'hover:bg-accent'} cursor-pointer`}>
                            <div className="flex items-center">
                              <input
                                type="checkbox"
                                checked={eventForm.assignedEmployees.includes(employee.id)}
                            ...
                            {hasConflict && (
                              <span className="text-xs px-1.5 py-0.5 bg-destructive/20 text-destructive rounded-full">Conflict</span>
                            )}
                          </label>
                        );
                      })}
